...
class HardwareStatus:
    ...
    def any_hardware(self) -> bool:
        ...
        return (
            self.tpm_available
            or self.yubikey_available
            or self.smartcard_available
            or self.sgx_available
        )
...
class HardwareKeyManager:
    ...
    def _detect_hardware(self) -> HardwareStatus:
        ...
        if not status.any_hardware():
            ...
            status.warnings.append("For maximum security, use a YubiKey or TPM-equipped system.")
    ...
    def derive_key_tpm(self, password: str, salt: bytes, key_length: int = 32) -> bytes:
        """
        ...
        try:
            ...
            success, output = self._run_command(
                ["tpm2_createprimary", "-C", "o", "-c", str(primary_ctx)]  # Owner hierarchy
            )
            ...
            # Get random from TPM
            success, output = self._run_command(["tpm2_getrandom", "--hex", "32"])
...
def add_hardware_key_args(parser):
    ...
    group.add_argument("--yubikey", action="store_true", help="Use YubiKey for key derivation")
    group.add_argument(
        "--yubikey-slot", type=int, default=2, choices=[1, 2], help="YubiKey slot (default: 2)"
    )
    group.add_argument("--tpm", action="store_true", help="Use TPM for key derivation")
    group.add_argument(
        "--hardware-auto", action="store_true", help="Automatically use best available hardware"
    )
    group.add_argument(
        "--check-hardware", action="store_true", help="Check and display hardware security status"
    )
