...
class ResumeManager:
    ...
    def load_state(
        self, session_id: str, password: Optional[str] = None, manifest: Optional[Manifest] = None
    ...
    def load_state_with_manifest(
        self, session_id: str, manifest: Manifest, password: str
    ) -> Optional[DecoderState]:
        """
        ...
        for state_file in self.state_dir.glob("session_*.meow"):
            try:
                ...
                try:
                    ...
                    if state:
                        solved_count = sum(
                            1 for _, block in state.solved_blocks if block is not None
                        )
        ...
        if not sessions:
            ...
            for s in sessions:
                ...
                if s["progress"] is not None:
                    ...
                    print(
                        f"    {Fore.YELLOW}(Encrypted - details require password){Style.RESET_ALL}"
                    )
                print()
