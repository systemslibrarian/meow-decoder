    ...
    def collect_kibble_streaming(self, seed: int, post_indices: List[int], data: bytes) -> bool:
        ...
        if len(indices) == 0:
            ...
            if self.posts_found % 10 == 0:
                print(
                    f"  🐾 Found {self.posts_found}/{self.num_posts} posts ({self.posts_found/self.num_posts*100:.1f}%)"
                )
...
def create_prowling_decoder(
    num_posts: int, post_size: int, max_ram_mb: int = 100
    ...
    config = ProwlingConfig(
        enable_low_ram=True, max_ram_mb=max_ram_mb, block_size=min(post_size, 256)
    )
    ...
    for i in range(max_kibbles):
        ...
        if kibbles_needed % 10 == 0:
            ...
            if stats["current_ram_mb"]:
                print(
                    f"  📊 Kibble {kibbles_needed}: RAM {stats['current_ram_mb']} MB, Pending {stats['pending_kibbles']}"
                )
