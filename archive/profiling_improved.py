...
class Profiler:
    ...
    def print_summary(self, top_n: int = 20, show_suggestions: bool = True):
        ...
        summary = self.get_summary()
        sorted_ops = sorted(summary.items(), key=lambda x: x[1]["total_time"], reverse=True)[:top_n]
        ...
        print(
            f"{'Operation':<30} {'Calls':>8} {'Total':>10} {'Avg':>10} {'Min':>10} {'Max':>10} {'%':>6}"
        )
        ...
        for name, data in sorted_ops:
            color = (
                ...
                else Fore.YELLOW if data["percentage"] > 10 else Fore.GREEN
            )
    ...
    def compare_with(self, other_profile: "Profiler", operation: str) -> None:
        ...
        if time_change < 0:
            print(
                f"  {Fore.GREEN}✓ Improvement: {abs(time_change):.1f}% faster ({speedup:.2f}x speedup){Style.RESET_ALL}"
            )
        else:
            print(
                f"  {Fore.RED}✗ Regression: {time_change:.1f}% slower ({1/speedup:.2f}x slowdown){Style.RESET_ALL}"
            )
    ...
    def save_profile(self, filename: str):
        ...
        summary["_suggestions"] = [
            {"category": cat, "suggestion": sug, "priority": pri} for cat, sug, pri in suggestions
        ]
