...
try:
    ...
    try:
        ...
        RESUME_AVAILABLE = True
        print(
            f"{Fore.YELLOW}⚠️  Using basic resume (not encrypted). Install cryptography for secure resume.{Style.RESET_ALL}"
        )
    except ImportError:
...
def check_for_existing_session(
    manifest: Manifest, password: str, resume_manager: ResumeManager
    ...
    try:
        ...
        response = (
            input(f"{Fore.GREEN}Resume from saved session? [Y/n]: {Style.RESET_ALL}")
            .strip()
            .lower()
        )
        ...
        if response in ("", "y", "yes"):
            ...
            print(
                f"{Fore.YELLOW}Starting fresh decode (old session kept for later)...{Style.RESET_ALL}\n"
            )
            return None
...
def _variants(frame_bgr: np.ndarray, include_aggressive: bool = False):
    ...
    ad = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 31, 3)
    yield ad
...
Resume Feature:
  ...
  • Encrypted state files (secure)
    ...
    parser.add_argument(
        "--output", default="cat_rebuilt.png", help="Path to save the reconstructed file"
    )
    parser.add_argument(
        "--camera-index", type=int, default=0, help="Webcam device index (default: 0)"
    )
    parser.add_argument("--seed", type=int, default=42069, help="Master seed (must match encoder)")
    parser.add_argument(
        "--metadata", action="store_true", help="Only read and verify manifest (no reconstruction)"
    )
    parser.add_argument(
        "--keyfile", type=str, default=None, help="Path to keyfile (must match encoder)"
    )
    parser.add_argument(
        "--aggressive",
        action="store_true",
    ...
    parser.add_argument(
        "--resume", action="store_true", default=True, help="Enable resume capability (default: on)"
    )
    parser.add_argument(
        "--no-resume", dest="resume", action="store_false", help="Disable resume capability"
    )
    parser.add_argument(
        "--auto-save-interval",
        type=int,
    ...
    if not args.quiet:
        ...
        if not RESUME_AVAILABLE:
            print(
                f"{Fore.YELLOW}⚠️  Resume capability not available (resume.py not found){Style.RESET_ALL}"
            )
    ...
    if args.keyfile:
        try:
            ...
            if not args.quiet:
                print(
                    f"{Fore.GREEN}✓ Keyfile loaded: {args.keyfile} ({len(keyfile)} bytes){Style.RESET_ALL}"
                )
        except Exception as e:
    ...
    if args.resume and RESUME_AVAILABLE:
        try:
            ...
            if not args.quiet:
                print(
                    f"{Fore.GREEN}✓ Resume capability enabled (auto-save every {args.auto_save_interval} droplets){Style.RESET_ALL}"
                )
        except Exception as e:
            if not args.quiet:
                print(
                    f"{Fore.YELLOW}⚠️  Resume capability initialization failed: {e}{Style.RESET_ALL}"
                )
            resume_manager = None
        ...
        while True:
            ...
            if text:
                ...
                if mm and decoder.manifest is None:
                    try:
                        ...
                        # NON-SECRET CHECKSUM ONLY — NOT PROTOCOL CRYPTO
                        # This HMAC check uses manifest-level authentication;
                        ...
                        if args.metadata:
                            ...
                            print(
                                f"{Fore.CYAN}Original length:{Style.RESET_ALL}   {format_size(decoder.manifest.orig_len)}"
                            )
                            print(
                                f"{Fore.CYAN}Blocks (k):{Style.RESET_ALL}        {decoder.manifest.k_blocks}"
                            )
                            sys.exit(0)
                        ...
                        if resume_manager and not resume_checked:
                            ...
                            decoder.decoder = FountainDecoder(
                                k=decoder.manifest.k_blocks, block_size=decoder.manifest.block_size
                            )
                            decoder.encoder_tmp = FountainEncoder(
                                b"\x00" * decoder.manifest.cipher_len,
                                block_size=decoder.manifest.block_size,
                        ...
                        if not args.quiet:
                            ...
                            print(
                                f"  K={decoder.manifest.k_blocks}, block_size={decoder.manifest.block_size} bytes"
                            )
                            if resumed_from_session:
                                solved = sum(1 for b in decoder.decoder.solved if b is not None)
                                print(
                                    f"  {Fore.CYAN}Resumed from saved session: {solved}/{decoder.manifest.k_blocks} blocks already solved{Style.RESET_ALL}"
                                )
            ...
            if not args.quiet and now - last_stats_print > 1.0:
                ...
                if decoder.manifest and decoder.decoder:
                    print(
                        f"\r{Fore.CYAN}Droplets: {stats['droplets_seen']}{Style.RESET_ALL} | "
                    ...
                    print(
                        f"\r{Fore.YELLOW}Scanning for manifest...{Style.RESET_ALL} "
            ...
            cv2.putText(
                frame,
                "Meow Decoder v2.1 - q=quit, a=aggressive, s=stats",
                (10, 30),
            ...
            if decoder.manifest and decoder.decoder:
                ...
                cv2.putText(
                    frame, status, (10, 65), cv2.FONT_HERSHEY_SIMPLEX, 0.8, overlay_color, 2
                )
    ...
    try:
        ...
        raw = decrypt_to_raw(
            cipher, password, decoder.manifest.salt, decoder.manifest.nonce, keyfile
        )
