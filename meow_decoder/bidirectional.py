...
class StatusUpdate:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "progress_percent": (
                100.0 * self.blocks_decoded / self.k_blocks_needed
                if self.k_blocks_needed > 0
                else 0.0
            ),
        }
...
class BiDirectionalProtocol:
    ...
    def start_session(
        self, file_hash: bytes, k_blocks: int, block_size: int, total_frames: int
    ) -> bytes:
        """
...
def create_sender_protocol(
    password: str, file_hash: bytes, k_blocks: int, block_size: int, total_frames: int
