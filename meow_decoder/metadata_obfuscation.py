...
def unshuffle_frames(shuffled_frames: List[bytes], shuffle_indices: List[int]) -> List[bytes]:
    """
...
def pad_frame_count(frames: List[bytes], target_count: int) -> List[bytes]:
    """
...
def obfuscate_encoding_parameters(
    block_size: int, redundancy: float, fps: int
) -> Tuple[int, float, int]:
    """
