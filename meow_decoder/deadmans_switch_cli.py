...
class DeadManSwitchState:
    ...
    def save(self):
        ...
        if self.state["last_checkin"] is None:
            ...
            next_deadline = datetime.now() + timedelta(
                seconds=self.checkin_interval + self.grace_period
            )
        else:
            # Renew: deadline is last checkin + interval + grace
            last_checkin = datetime.fromisoformat(self.state["last_checkin"])
            next_deadline = last_checkin + timedelta(
                seconds=self.checkin_interval + self.grace_period
            )
...
📖 DOCUMENTATION:
  ...
  See docs/THREAT_MODEL.md § Coercion Resistance for full details
    ...
    setup_parser.add_argument(
        "--duration", default="24h", help="Check-in interval (e.g., 24h, 7d, 3600s)"
    )
    setup_parser.add_argument("--decoy", help="Path to decoy file to auto-release")
    setup_parser.add_argument(
        "--grace", type=int, default=1, help="Grace period in hours (default: 1)"
    )
    ...
    trigger_parser.add_argument("--confirm", action="store_true", help="Skip confirmation prompt")
    ...
    disable_parser.add_argument("--confirm", action="store_true", help="Skip confirmation prompt")
