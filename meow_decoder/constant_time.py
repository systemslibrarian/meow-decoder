...
def _set_dontdump(addr: int, size: int) -> bool:
    ...
    Uses madvise(MADV_DONTDUMP) to prevent the region from appearing
    in core dumps generated by the kernel.

    Note: madvise requires page-aligned addresses. This function aligns
    the address down to the nearest page boundary and extends the size
    ...
    """
    if platform.system() != "Linux" or _libc is None:
        return False
    try:
        import os
        ...
        # Extend size to cover from aligned address through original end
        aligned_size = (addr - aligned_addr) + size
...
def secure_memory(data: bytes) -> Iterator[bytearray]:
    ...
    # Try to lock in RAM and exclude from core dumps
    locked = False
    if _libc is not None:
        try:
            ...
            locked = result == 0
            # Also exclude from core dumps (Linux)
            if locked:
                _set_dontdump(ctypes.addressof(addr), len(buf))
        except Exception:
            pass
    ...
    def __init__(self, size: int):
        ...
        if _libc is not None:
            try:
                ...
                if self.locked:
                    self.dontdump = _set_dontdump(ctypes.addressof(addr), size)
            except Exception:
                pass
