...
def hiss_file_into_yarn_ball(
    ...
    if verbose:
        print(f"  🗜️  Compressed: {len(compressed):,} bytes ({len(compressed)/len(secret_data)*100:.1f}%)")
...
🐾 Strong cat passwords only! 🔐
    ...
    parser.add_argument('-i', '--input', type=Path, required=True,
                       help='🐱 Input file to hiss')
    parser.add_argument('-o', '--output', type=Path, required=True,
                       help='🧶 Output yarn ball (GIF)')
    
    # Optional arguments
    parser.add_argument('-p', '--password', type=str,
                       help='🔐 Strong cat password (prompted if not provided)')
    parser.add_argument('-c', '--catnip', type=Path,
                       help='🌿 Path to catnip file (keyfile)')
    
    # Encoding parameters
    parser.add_argument('--post-size', type=int, default=512,
                       help='📏 Scratching post size (default: 512)')
    parser.add_argument('--kibbles', type=float, default=1.5,
                       help='🍖 Kibble redundancy factor (default: 1.5)')
    parser.add_argument('--fps', type=int, default=10,
                       help='🎬 Yarn ball frames per second (default: 10)')
    ...
    parser.add_argument('--paw-size', type=int, default=10,
                       help='🐾 Paw print box size (default: 10)')
    parser.add_argument('--paw-border', type=int, default=4,
                       help='🐾 Paw print border size (default: 4)')
    
    # Output control
    parser.add_argument('-v', '--verbose', action='store_true',
                       help='😸 Verbose kitty output')
    parser.add_argument('--shred-source', action='store_true',
                       help='🔥 Securely shred source file after encoding')
