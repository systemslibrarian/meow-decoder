...
class LogoEyesEncoder:
    ...
    def _create_placeholder_logo(self) -> Image.Image:
        ...
        draw.ellipse(
            [
                ORIGINAL_LEFT_EYE[0] - 30,
                ORIGINAL_LEFT_EYE[1] - 30,
                ORIGINAL_LEFT_EYE[0] + 30,
                ORIGINAL_LEFT_EYE[1] + 30,
            ],
            outline=(0, 255, 100),
        ...
        draw.ellipse(
            [
                ORIGINAL_RIGHT_EYE[0] - 30,
                ORIGINAL_RIGHT_EYE[1] - 30,
                ORIGINAL_RIGHT_EYE[0] + 30,
                ORIGINAL_RIGHT_EYE[1] + 30,
            ],
            outline=(0, 255, 100),
        ...
        # Ears
        draw.polygon(
            [(w // 4, h // 4), (w // 4 - 50, h // 8), (w // 4 + 50, h // 4)],
            outline=(255, 255, 255),
        )
        draw.polygon(
            [(3 * w // 4, h // 4), (3 * w // 4 + 50, h // 8), (3 * w // 4 - 50, h // 4)],
            outline=(255, 255, 255),
        )
        # Text
...
class LogoEyesDecoder:
    ...
    def _extract_from_region(
        self, carrier: np.ndarray, x: int, y: int, w: int, h: int, lsb_bits: int
    ) -> np.ndarray:
...
def decode_from_logo_eyes(
    frames: List[Image.Image], config: Optional[LogoConfig] = None, lsb_bits: int = 2
) -> List[Image.Image]:
    """
...
if __name__ == "__main__":
    ...
    print(
        f"   Left eye: ({encoder.left_eye.center_x}, {encoder.left_eye.center_y}) r={encoder.left_eye.radius}"
    )
    print(
        f"   Right eye: ({encoder.right_eye.center_x}, {encoder.right_eye.center_y}) r={encoder.right_eye.radius}"
    )
