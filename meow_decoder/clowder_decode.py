...
def decode_clowder(
    clowder_dir: Path, output_dir: Path, password: str, verbose: bool = False
) -> dict:
    """
    ...
    for i, yarn_ball in enumerate(manifest["yarn_balls"], 1):
        ...
        try:
            decode_gif(str(yarn_ball_file), str(temp_output), password, verbose=False)
...
The clowder reunites! 🐈🐈🐈
    ...
    parser.add_argument(
        "--input", type=Path, required=True, help="Clowder directory (with manifest)"
    )
    parser.add_argument(
        "--output", type=Path, required=True, help="Output directory for recovered files"
    )
    parser.add_argument(
        "--password", type=str, help="Decryption password (prompted if not provided)"
    )
    ...
    # Decode!
    try:
        stats = decode_clowder(args.input, args.output, password, verbose=args.verbose)
