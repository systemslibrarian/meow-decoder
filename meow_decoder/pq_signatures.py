...
class Signature:
    ...
    def unpack(cls, data: bytes) -> "Signature":
        ...
        if algorithm == SIG_HYBRID:
            ...
            return cls(algorithm=algorithm, signature=data[1:])
...
def _generate_ed25519_keypair() -> SignatureKeyPair:
    ...
    return SignatureKeyPair(
        algorithm=SIG_ED25519, private_key=private_bytes, public_key=public_bytes
    )
...
def _generate_dilithium_keypair() -> SignatureKeyPair:
    ...
    return SignatureKeyPair(
        algorithm=SIG_DILITHIUM3, private_key=private_key, public_key=public_key
    )
...
def _sign_ed25519(data: bytes, private_key: bytes) -> Signature:
    ...
    return Signature(algorithm=SIG_ED25519, signature=sig)
...
def _sign_dilithium(data: bytes, private_key: bytes) -> Signature:
    ...
    return Signature(algorithm=SIG_DILITHIUM3, signature=sig)
...
def verify_manifest(manifest_bytes: bytes, signature: Signature, public_key: bytes) -> bool:
    """
...
def save_keypair(
    keypair: SignatureKeyPair, private_path: str, public_path: str, password: Optional[str] = None
):
    """
...
def load_keypair(
    private_path: str, public_path: str, password: Optional[str] = None
) -> SignatureKeyPair:
    """
    ...
    if algorithm == SIG_HYBRID:
        ...
        return SignatureKeyPair(algorithm=algorithm, private_key=private_key, public_key=public_key)
