...
def encode_clowder(
    ...
    if resume and manifest_path.exists():
        ...
        # Create new clowder
        clowder_id = hashlib.sha256(f"{input_path}{datetime.now()}".encode()).hexdigest()[:16]

        clowder_manifest = ClowderManifest(
            clowder_id=clowder_id, password_hash=hash_password(password)
        )
    ...
    for i in range(0, len(remaining_files), max_files_per_yarn):
        ...
        try:
            stats = encode_file(temp_file, yarn_ball_path, password, config=config, verbose=False)
    ...
    for yarn_ball in clowder_manifest.yarn_balls:
        print(
            f"  {yarn_ball['filename']}: {len(yarn_ball['files'])} files, {yarn_ball['qr_frames']} frames"
        )
...
A clowder is a group of cats! 🐈🐈🐈
    ...
    parser.add_argument(
        "--output", type=Path, required=True, help="Output directory for yarn balls"
    )
    parser.add_argument(
        "--password", type=str, help="Encryption password (prompted if not provided)"
    )
    parser.add_argument(
        "--max-per-yarn", type=int, default=10, help="Maximum files per yarn ball (default: 10)"
    )
    parser.add_argument(
        "--no-resume", action="store_true", help="Start fresh (ignore existing manifest)"
    )
    parser.add_argument(
    ...
    # Create config
    config = EncodingConfig(block_size=args.block_size, fps=args.fps)
