...
class CatNapDistribution:
    ...
    def _calculate_nap_schedule(self):
        ...
        spike_index = max(
            1, min(int(self.num_posts / R) if R > 0 else self.num_posts, self.num_posts)
        )
...
class CatnipFountain:
    ...
    def drop_kibble(self, seed: Optional[int] = None) -> Kibble:
        ...
        return Kibble(seed=seed, scratching_post_indices=post_indices, data=bytes(kibble_data))
...
def unpack_kibble(data: bytes, post_size: int) -> Kibble:
    ...
    return Kibble(seed=seed, scratching_post_indices=indices, data=kibble_data)
