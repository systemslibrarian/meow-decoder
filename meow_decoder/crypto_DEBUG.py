...
def unpack_manifest(b: bytes) -> Manifest:
    ...
    if len(b) not in [min_len, fs_len, pq_len]:
        raise ValueError(
            f"Manifest length invalid (got {len(b)}, expected {min_len}, {fs_len}, or {pq_len} bytes)"
        )
...
def compute_manifest_hmac(
    ...
    # Otherwise derive key based on encryption mode (decoding path)
        ...
        if debug:
            print(
                f"    Calling derive_shared_secret(receiver_privkey, sender_pubkey, password, salt)"
            )
