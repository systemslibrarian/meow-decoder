...
class HardwareCapabilities:
    ...
    def summary(self) -> str:
        ...
        lines = ["🔐 Hardware Security Status", "=" * 40]
...
class HardwareSecurityProvider:
        ...
        if os.path.exists("/dev/tpm0") or os.path.exists("/dev/tpmrm0"):
            ...
            if ok:
                ...
                version_match = re.search(
                    r"TPM2_PT_FAMILY_INDICATOR.*value:\s*\"([^\"]+)\"", output
                )
                if version_match:
    ...
    def tpm_seal(
        ...
        with tempfile.TemporaryDirectory() as tmpdir_str:
            ...
            cmd = [
                ...
                str(data_file),
                "-u",
                str(tmpdir / "sealed.pub"),
                "-r",
                str(tmpdir / "sealed.priv"),
                "-L",
                f"sha256:{pcr_list}",
            ]
    ...
    def tpm_unseal(self, sealed_blob: bytes, auth_password: Optional[str] = None) -> bytes:
        ...
        with tempfile.TemporaryDirectory() as tmpdir_str:
            ...
            cmd = ["tpm2_startauthsession", "--policy-session", "-S", str(tmpdir / "session.ctx")]
            ok, output = self._run_cmd(cmd, timeout=30)
            if not ok:
                raise HardwareOperationError(f"TPM session failed: {output}")

            cmd = ["tpm2_policypcr", "-S", str(tmpdir / "session.ctx"), "-l", f"sha256:{pcr_list}"]
            ok, output = self._run_cmd(cmd, timeout=30)
            if not ok:
    ...
    def hsm_generate_key(
        self, slot: int, pin: str, key_label: str = "meow-master", key_type: str = "aes256"
    ) -> str:
        """
    ...
    def derive_key_auto(
        self, password: bytes, salt: bytes, prefer: Optional[HardwareType] = None, **kwargs
        ...
        if hw_type == HardwareType.YUBIKEY_PIV and caps.yubikey_available:
            key = self.derive_key_yubikey_piv(
                password, salt, kwargs.get("yubikey_slot", "9d"), kwargs.get("yubikey_pin")
            )
            return key, HardwareType.YUBIKEY_PIV

        if hw_type == HardwareType.TPM and caps.tpm_available:
            key = self.derive_key_tpm(password, salt, kwargs.get("pcrs"))
...
def add_hardware_args(parser) -> None:
    ...
    group.add_argument(
        "--yubikey-piv", action="store_true", help="Use YubiKey PIV for key derivation"
    )
    group.add_argument(
        "--yubikey-fido2",
        action="store_true",
    ...
    group.add_argument("--tpm-derive", action="store_true", help="Use TPM for key derivation")

    # Auto/detection options
    group.add_argument(
        "--hardware-auto", action="store_true", help="Automatically use best available hardware"
    )
    group.add_argument(
...
def process_hardware_args(args, password: bytes, salt: bytes) -> Tuple[bytes, str]:
    ...
    if getattr(args, "hsm_slot", None) is not None:
        ...
        key = provider.hsm_derive_key(
            password, salt, args.hsm_slot, pin, getattr(args, "hsm_key_label", "meow-master")
        )
    ...
    if getattr(args, "yubikey_piv", False) or getattr(args, "yubikey", False):
        ...
        key = provider.derive_key_yubikey_piv(password, salt, slot, pin)
