...
class MeowGUI:
    ...
    def __init__(self):
        ...
        # Create viewport
        dpg.create_viewport(title="🐱 Meow Decoder - Enhanced Dashboard", width=1200, height=800)
        ...
        with dpg.window(label="Meow Decoder Dashboard", tag="main_window"):
            ...
            with dpg.tab_bar():
                ...
                with dpg.tab(label="📤 Encode"):
                    ...
                    with dpg.group(horizontal=True):
                        dpg.add_input_text(
                            tag="encode_output", width=400, default_value="output.gif"
                        )
                    ...
                    # Security options
                    dpg.add_checkbox(
                        label="Forward Secrecy (MEOW3)", tag="encode_fs", default_value=True
                    )
                    dpg.add_checkbox(label="Post-Quantum (MEOW4)", tag="encode_pq")
                    dpg.add_slider_int(
                        label="Steganography Level",
                        tag="encode_stego",
                        default_value=0,
                        min_value=0,
                        max_value=4,
                    ...
                    # Encode button
                    dpg.add_button(
                        label="🐱 ENCODE FILE", callback=self.encode_file, width=200, height=40
                    )
                    ...
                    dpg.add_spacer(height=10)
                    dpg.add_input_text(
                        tag="encode_log", multiline=True, readonly=True, height=200, width=800
                    )
                ...
                with dpg.tab(label="📥 Decode"):
                    ...
                    with dpg.group(horizontal=True):
                        dpg.add_input_text(
                            tag="decode_output", width=400, default_value="decrypted_output"
                        )
                    ...
                    # Options
                    dpg.add_checkbox(
                        label="Nine Lives Retry Mode", tag="decode_retry", default_value=True
                    )
                    ...
                    # Decode button
                    dpg.add_button(
                        label="🔓 DECODE FILE", callback=self.decode_file, width=200, height=40
                    )
                    ...
                    dpg.add_spacer(height=10)
                    dpg.add_input_text(
                        tag="decode_log", multiline=True, readonly=True, height=200, width=800
                    )
                ...
                if HAS_OPENCV:
                    with dpg.tab(label="📷 Webcam Decode"):
                        ...
                        dpg.add_spacer(height=10)
                        dpg.add_input_text(
                            tag="webcam_log", multiline=True, readonly=True, height=150, width=800
                        )
    ...
    def encode_file(self):
        ...
        thread = threading.Thread(
            target=self._encode_thread, args=(input_file, output_file, password)
        )
        thread.daemon = True
        thread.start()
    ...
    def decode_file(self):
        ...
        thread = threading.Thread(
            target=self._decode_thread, args=(input_file, output_file, password)
        )
        thread.daemon = True
        thread.start()
