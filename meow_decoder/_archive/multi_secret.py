...
class MultiSecretEncoder:
    ...
    def encode(self) -> Tuple[bytes, MultiSecretManifest]:
        ...
        all_blocks = [self._pad_to_blocks(ct, blocks_per_reality) for ct in ciphertexts]
...
class MultiSecretDecoder:
    ...
    def _verify_password(self, password: str) -> int:
        ...
        for i in range(self.manifest.n_realities):
            ...
            if _get_backend().constant_time_compare(expected_hmac, self.manifest.hmacs[i]):
                return i
...
def encode_multi_secret(
    secrets: List[Tuple[bytes, str]], block_size: int = 256
...
def decode_multi_secret(
    superposition: bytes, manifest: MultiSecretManifest, password: str
) -> bytes:
    """
...
if __name__ == "__main__":
    ...
    secrets_5 = [(f"Secret level {i}: {'x' * 1000}".encode(), f"password{i}") for i in range(5)]
