...
class SecureBridge:
    ...
    def create_key_handle(
        self, password: str, salt: bytes, memory_kib: int = 524288, iterations: int = 20  # 512 MiB
    ) -> KeyHandle:
        """
        ...
        try:
            ...
            handle = KeyHandle(_handle_id=handle_id, _backend="rust", _key_bytes=key)
    ...
    def encrypt_with_handle(
        self, handle: KeyHandle, plaintext: bytes, aad: Optional[bytes] = None
    ) -> Tuple[bytes, bytes]:
        """
    ...
    def decrypt_with_handle(
        self, handle: KeyHandle, nonce: bytes, ciphertext: bytes, aad: Optional[bytes] = None
    ) -> bytes:
        """
