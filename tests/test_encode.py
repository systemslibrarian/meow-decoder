...
def _patch_encode_pipeline(monkeypatch):
    ...
    monkeypatch.setattr(enc, "QRCodeGenerator", lambda *a, **k: type("Q", (), {"generate": lambda s, p: _DummyQR()})())
    monkeypatch.setattr(enc, "GIFEncoder", lambda *a, **k: type("G", (), {"create_gif": lambda s, f, p, **kw: (p.write_bytes(b"GIF89a"), p.stat().st_size)[1]})())
...
def _patch_encode_file(monkeypatch, record=None):
    ...
    def _fake(*args, **kwargs):
        ...
        return {
            "input_size": 4, "compressed_size": 4, "encrypted_size": 4,
            "output_size": 10, "compression_ratio": 1.0, "k_blocks": 1,
            "num_droplets": 1, "redundancy": 1.5, "qr_frames": 1,
            "qr_size": (64, 64), "gif_duration": 0.1, "elapsed_time": 0.01,
        }
...
def test_encode_file_duress_requires_forward_secrecy(tmp_path):
    ...
    with pytest.raises(ValueError, match="Duress mode requires forward secrecy"):
        enc.encode_file(inp, tmp_path / "out.gif", password="pw", duress_password="dur", forward_secrecy=False)
...
def test_encode_file_duress_requires_pubkey_or_pq(tmp_path):
    ...
    with pytest.raises(ValueError, match="Duress mode requires a distinct manifest format"):
        enc.encode_file(inp, tmp_path / "out.gif", password="pw", duress_password="dur",
                        forward_secrecy=True, receiver_public_key=None, use_pq=False)
...
def test_encode_file_verbose_meow2_manifest_print(monkeypatch, tmp_path, capsys):
    ...
    inp = _write_input(tmp_path)
    enc.encode_file(inp, tmp_path/"out.gif", password="password1", forward_secrecy=False, verbose=True)
...
def test_main_keyfile_not_found_exits_1(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(sys, "argv", [
        "meow-encode", "-i", str(inp), "-o", str(tmp_path/"out.gif"),
        "-p", "password1", "-k", str(tmp_path/"missing.key")
    ])
    with pytest.raises(SystemExit) as exc:
        enc.main()
    assert exc.value.code == 1
...
def test_main_wipe_source_secure_wipe_fallback(monkeypatch, tmp_path):
    ...
    def _fake_import(name, *args, **kwargs):
        if name == "meow_decoder.high_security" or (len(args) > 2 and args[2] and "secure_wipe_file" in args[2]):
...
def test_main_catnip_flavor_verbose(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(sys, "argv", [
        "meow-encode", "-i", str(inp), "-o", str(tmp_path/"out.gif"),
        "-p", "password1", "--catnip", "tuna", "-v", "--no-forward-secrecy"
    ])
    enc.main()
    captured = capsys.readouterr()
...
def test_main_nine_lives_retry_success_after_failures(monkeypatch, tmp_path, capsys):
    ...
    _install_module(monkeypatch, "meow_decoder.cat_utils",
                    NineLivesRetry=_NineLivesRetry, meow_about=lambda: "about")
    monkeypatch.setattr(enc, "encode_file", _failing_encode)
    monkeypatch.setattr(sys, "argv", [
        "meow-encode", "-i", str(inp), "-o", str(tmp_path/"out.gif"),
        "-p", "password1", "--nine-lives"
    ])
    enc.main()
...
def test_main_deadmans_switch_setup_exception_swallowed(monkeypatch, tmp_path, capsys):
    ...
    _install_module(monkeypatch, "meow_decoder.deadmans_switch_cli",
                    DeadManSwitchState=_BadDeadMan)
    monkeypatch.setattr(sys, "argv", [
        "meow-encode", "-i", str(inp), "-o", str(tmp_path/"out.gif"),
        "-p", "password1", "--dead-mans-switch", "24h", "-v"
    ])
    enc.main()
    captured = capsys.readouterr()
...
def test_encode_file_precomputed_key_wrong_length(tmp_path):
    ...
    with pytest.raises((ValueError, RuntimeError)):
        enc.encode_file(inp, tmp_path/"out.gif", password="pw",
                        hardware_key=b"short", hardware_salt=b"s"*16)
...
def test_encode_file_yubikey_with_keyfile_rejected(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(sys, "argv", [
        "meow-encode", "-i", str(inp), "-o", str(tmp_path/"out.gif"),
        "-p", "password1", "-k", str(keyfile), "--yubikey"
    ])
    with pytest.raises(SystemExit) as exc:
        enc.main()
    assert exc.value.code == 1
...
def test_main_hsm_with_keyfile_rejected(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(sys, "argv", [
        "meow-encode", "-i", str(inp), "-o", str(tmp_path/"out.gif"),
        "-p", "password1", "-k", str(keyfile), "--hsm-slot", "0"
    ])
    with pytest.raises(SystemExit) as exc:
        enc.main()
    assert exc.value.code == 1
...
def test_main_tpm_with_keyfile_rejected(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(sys, "argv", [
        "meow-encode", "-i", str(inp), "-o", str(tmp_path/"out.gif"),
        "-p", "password1", "-k", str(keyfile), "--tpm-derive"
    ])
    with pytest.raises(SystemExit) as exc:
        enc.main()
    assert exc.value.code == 1
...
def test_main_hardware_auto_with_keyfile_rejected(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(sys, "argv", [
        "meow-encode", "-i", str(inp), "-o", str(tmp_path/"out.gif"),
        "-p", "password1", "-k", str(keyfile), "--hardware-auto"
    ])
    with pytest.raises(SystemExit) as exc:
        enc.main()
    assert exc.value.code == 1
...
def test_main_about_flag_exits_0(monkeypatch, capsys):
    ...
    _install_module(monkeypatch, "meow_decoder.cat_utils",
                    meow_about=_fake_meow_about, enable_purr_mode=lambda enabled: None)
...
def test_main_hardware_status_flag_exits_0(monkeypatch, capsys):
    ...
    _install_module(monkeypatch, "meow_decoder.hardware_integration",
                    HardwareSecurityProvider=_FakeProvider, process_hardware_args=lambda *a: (None, "none"))
...
def test_main_safety_checklist_flag_exits_0(monkeypatch, capsys):
    ...
    _install_module(monkeypatch, "meow_decoder.high_security",
                    get_safety_checklist=_fake_checklist)
...
def test_main_purr_mode_enables_verbose(monkeypatch, tmp_path, capsys):
    ...
    _install_module(monkeypatch, "meow_decoder.cat_utils",
                    enable_purr_mode=lambda enabled: None, meow_about=lambda: "about")
    _patch_encode_file(monkeypatch)
    monkeypatch.setattr(sys, "argv", [
        "meow-encode", "-i", str(inp), "-o", str(tmp_path/"out.gif"),
        "-p", "password1", "--purr-mode"
    ])
    enc.main()
...
def test_main_input_is_directory_exits_1(monkeypatch, tmp_path):
    ...
    input_dir.mkdir()
    monkeypatch.setattr(sys, "argv", [
        "meow-encode", "-i", str(input_dir), "-o", str(tmp_path/"out.gif"),
        "-p", "password1"
    ])
    with pytest.raises(SystemExit) as exc:
        enc.main()
...
def test_main_pq_mode_verbose_print(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(sys, "argv", [
        "meow-encode", "-i", str(inp), "-o", str(tmp_path/"out.gif"),
        "-p", "password1", "--pq", "--no-forward-secrecy", "-v"
    ])
    enc.main()
    captured = capsys.readouterr()
...
def test_main_receiver_pubkey_success_print(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(sys, "argv", [
        "meow-encode", "-i", str(inp), "-o", str(tmp_path/"out.gif"),
        "-p", "password1", "--receiver-pubkey", str(pubkey)
    ])
    enc.main()
    captured = capsys.readouterr()
...
def test_main_duress_password_same_as_main_exits_1(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(sys, "argv", [
        "meow-encode", "-i", str(inp), "-o", str(tmp_path/"out.gif"),
        "-p", "samepw", "--duress-password", "samepw"
    ])
    with pytest.raises(SystemExit) as exc:
        enc.main()
    assert exc.value.code == 1
...
def test_main_duress_cli_requires_fs_exits_1(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(sys, "argv", [
        "meow-encode", "-i", str(inp), "-o", str(tmp_path/"out.gif"),
        "-p", "mainpw", "--duress-password", "duress", "--no-forward-secrecy"
    ])
    with pytest.raises(SystemExit) as exc:
        enc.main()
    assert exc.value.code == 1
...
def test_main_hardware_key_derivation_success(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(sys, "argv", [
        "meow-encode", "-i", str(inp), "-o", str(tmp_path/"out.gif"),
        "-p", "password1", "--yubikey", "--yubikey-pin", "123456", "-v"
    ])
    enc.main()
    captured = capsys.readouterr()
    assert "Key derived via" in captured.out or "YubiKey" in captured.out or "Output saved to" in captured.out
...
def test_main_hardware_key_derivation_returns_none_fallback(monkeypatch, tmp_path, capsys):
    ...
    enc.main()
    captured = capsys.readouterr()
    assert "falling back" in captured.out.lower() or "software" in captured.out.lower() or "Output saved to" in captured.out
...
def test_main_hardware_key_derivation_exception_no_fallback_exits_1(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(sys, "argv", [
        "meow-encode", "-i", str(inp), "-o", str(tmp_path/"out.gif"),
        "-p", "password1", "--hsm-slot", "0", "--hsm-pin", "123456", "--no-hardware-fallback"
    ])
    with pytest.raises(SystemExit) as exc:
        enc.main()
    assert exc.value.code == 1
...
def test_main_hardware_key_derivation_exception_with_fallback(monkeypatch, tmp_path, capsys):
    ...
    enc.main()
    captured = capsys.readouterr()
    assert "Falling back" in captured.out or "software" in captured.out.lower() or "failed" in captured.out.lower() or "Output saved to" in captured.out
...
def test_encode_file_verbose_meow3_with_pubkey_print(monkeypatch, tmp_path, capsys):
    ...
    inp = _write_input(tmp_path)
    enc.encode_file(inp, tmp_path/"out.gif", password="password1",
                    forward_secrecy=True, receiver_public_key=b"r"*32, verbose=True)
    captured = capsys.readouterr()
...
def test_encode_file_verbose_meow3_password_only_print(monkeypatch, tmp_path, capsys):
    ...
    inp = _write_input(tmp_path)
    enc.encode_file(inp, tmp_path/"out.gif", password="password1",
                    forward_secrecy=True, receiver_public_key=None, verbose=True)
    captured = capsys.readouterr()
...
def test_encode_file_yubikey_kwargs_passed(monkeypatch, tmp_path):
    ...
    inp = _write_input(tmp_path)
    enc.encode_file(inp, tmp_path/"out.gif", password="password1",
                    yubikey=True, yubikey_slot="9a", yubikey_pin="1234")
...
def test_encode_file_verbose_duress_print(monkeypatch, tmp_path, capsys):
    ...
    inp = _write_input(tmp_path)
    enc.encode_file(inp, tmp_path/"out.gif", password="password1",
                    duress_password="duress", forward_secrecy=True,
                    receiver_public_key=b"r"*32, verbose=True)
    captured = capsys.readouterr()
...
def test_encode_file_verbose_ephemeral_key_print(monkeypatch, tmp_path, capsys):
    ...
    captured = capsys.readouterr()
    assert "Ephemeral" in captured.out or "Forward secrecy" in captured.out or "32 bytes" in captured.out
...
def test_encode_file_secure_zero_exception_swallowed(monkeypatch, tmp_path):
    ...
    _install_module(monkeypatch, "meow_decoder.crypto_backend",
                    get_default_backend=lambda: _FakeBackend())
...
def test_main_fs_enabled_verbose_print(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(sys, "argv", [
        "meow-encode", "-i", str(inp), "-o", str(tmp_path/"out.gif"),
        "-p", "password1", "--receiver-pubkey", str(pubkey), "-v"
    ])
    enc.main()
    captured = capsys.readouterr()
...
def test_main_fs_config_on_no_receiver_verbose_print(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(sys, "argv", [
        "meow-encode", "-i", str(inp), "-o", str(tmp_path/"out.gif"),
        "-p", "password1", "-v"  # No --receiver-pubkey, default FS on
    ])
    enc.main()
    captured = capsys.readouterr()
...
def test_main_fs_disabled_verbose_print(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(sys, "argv", [
        "meow-encode", "-i", str(inp), "-o", str(tmp_path/"out.gif"),
        "-p", "password1", "--no-forward-secrecy", "-v"
    ])
    enc.main()
    captured = capsys.readouterr()
...
def test_main_fs_enabled_no_receiver_prints_warning(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(sys, "argv", [
        "meow-encode", "-i", str(inp), "-o", str(tmp_path/"out.gif"),
        "-p", "password1"  # Default FS on, no --receiver-pubkey
    ])
    enc.main()
    captured = capsys.readouterr()
    assert "no receiver public key" in captured.out.lower() or "password-only" in captured.out.lower()
