    ...
    def test_rust_backend_yubikey_missing_raises_runtime_error(self):
        ...
        with pytest.raises(RuntimeError, match="YubiKey|not available"):
            backend.derive_key_yubikey(
                password=b"password", salt=secrets.token_bytes(16), slot="9d", pin="123456"
            )
...
class TestPropertyBased:
    ...
    @given(
        plaintext=st.binary(min_size=1, max_size=10000), aad=st.binary(min_size=0, max_size=1000)
    )
    ...
    @given(key=st.binary(min_size=32, max_size=32), data=st.binary(min_size=1, max_size=10000))
    @settings(max_examples=50, deadline=5000)
    ...
    @given(a=st.binary(min_size=0, max_size=1000), b=st.binary(min_size=0, max_size=1000))
    ...
    @given(length=st.integers(min_value=1, max_value=256))
    @settings(max_examples=50, deadline=5000)
...
class TestAdversarial:
    ...
    def test_aes_gcm_wrong_aad_fails(self, random_key, random_nonce, sample_plaintext):
        ...
        ciphertext = backend.aes_gcm_encrypt(
            random_key, random_nonce, sample_plaintext, b"correct aad"
        )
