    ...
    def test_timelock_puzzle(self):
        ...
        try:
            ...
            config = TimeLockConfig(
                lock_duration_seconds=1,  # Fast for testing
                hash_iterations_per_second=100  # Low iterations for fast test
            )
            puzzle = TimeLockPuzzle(config)
            
            assert puzzle is not None
            assert puzzle.config.lock_duration_seconds == 1
        except ImportError:
    ...
    def test_countdown_duress(self):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                state_path = Path(tmpdir) / "countdown_state.json"
                duress = CountdownDuress(config, state_path)
                
                assert duress is not None
                assert duress.config.checkin_interval_seconds == 3600
        except ImportError:
    ...
    def test_dead_man_switch(self):
        ...
        try:
            ...
            config = TimeLockConfig(
                ...
                deadman_duration_days=1  # 1 day for testing
            )
            
            with tempfile.TemporaryDirectory() as tmpdir:
                state_path = Path(tmpdir) / "deadman_state.json"
                switch = DeadManSwitch(config, state_path)
                
                assert switch is not None
                assert switch.config.deadman_duration_days == 1
        except ImportError:
