...
class TestPQFailClosed:
    """Tests for fail-closed behavior (from test_pq_hybrid_fail_closed.py)."""
    ...
    def test_hybrid_encapsulate_fails_if_pq_requested_but_unavailable(self, monkeypatch):
        """Test that encapsulate fails if PQ requested but liboqs unavailable."""
        from meow_decoder import pq_hybrid
        ...
        with pytest.raises(RuntimeError):
            pq_hybrid.hybrid_encapsulate(
                receiver_classical_public=self._valid_x25519_public_key_bytes(),
                receiver_pq_public=b"\x00" * 1568,
            )
    
    def test_hybrid_encapsulate_allows_classical_only_when_pq_not_requested(self, monkeypatch):
        """Test that classical-only mode works when PQ not requested."""
        from meow_decoder import pq_hybrid
    ...
    def test_hybrid_decapsulate_fails_if_pq_ciphertext_without_pq_key(self):
        """Test that decapsulate fails if PQ ciphertext present but no PQ key."""
        ...
        with pytest.raises(RuntimeError):
            hybrid_decapsulate(
                ephemeral_classical_public=self._valid_x25519_public_key_bytes(),
