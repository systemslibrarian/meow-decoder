...
def test_main_input_missing_exits(tmp_path):
    ...
    with patch("sys.argv", ["meow-decode-gif", "-i", "missing.gif", "-o", str(out_path), "-p", "password123"]):
        with pytest.raises(SystemExit):
            decode_mod.main()
...
def test_main_input_is_directory_exits(tmp_path):
    ...
    with patch("sys.argv", ["meow-decode-gif", "-i", str(input_path), "-o", str(output_path), "-p", "password123"]):
        with pytest.raises(SystemExit):
            decode_mod.main()
...
def test_main_empty_password_exits(tmp_path):
    ...
    with patch("sys.argv", ["meow-decode-gif", "-i", str(input_path), "-o", str(output_path), "-p", ""]):
        with pytest.raises(SystemExit):
            decode_mod.main()
...
def test_main_success_calls_decode(tmp_path, monkeypatch):
    ...
    def _fake_decode(*args, **kwargs):
        return {"input_frames": 1, "qr_codes_read": 1, "droplets_processed": 1, "blocks_decoded": 1, "output_size": 4, "efficiency": 1.0, "elapsed_time": 0.1}

    monkeypatch.setattr(decode_mod, "decode_gif", _fake_decode)

    with patch("sys.argv", ["meow-decode-gif", "-i", str(input_path), "-o", str(output_path), "-p", "password123", "--force"]):
        decode_mod.main()
...
def test_main_password_prompt_path(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(decode_mod, "decode_gif", lambda *args, **kwargs: {"input_frames": 1, "qr_codes_read": 1, "droplets_processed": 1, "blocks_decoded": 1, "output_size": 4, "efficiency": 1.0, "elapsed_time": 0.1})
    monkeypatch.setattr(decode_mod, "getpass", lambda *args, **kwargs: "password123")

    with patch("sys.argv", ["meow-decode-gif", "-i", str(input_path), "-o", str(output_path), "--force"]):
        decode_mod.main()
...
def test_main_receiver_privkey_invalid(tmp_path):
    ...
    with patch("sys.argv", [
        ...
        "-o", str(output_path),
        "-p", "password123",
        "--receiver-privkey", str(privkey_path),
        "--receiver-privkey-password", "pass",
    ]):
        with pytest.raises(SystemExit):
            decode_mod.main()
...
def test_main_keyfile_error_exits(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(decode_mod, "verify_keyfile", lambda *args, **kwargs: (_ for _ in ()).throw(ValueError("bad")))

    with patch("sys.argv", ["meow-decode-gif", "-i", str(input_path), "-o", str(output_path), "-p", "password123", "--keyfile", str(tmp_path / "missing.bin")]):
        with pytest.raises(SystemExit):
            decode_mod.main()
...
def test_main_purr_mode_nine_lives_success(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("meow_decoder.cat_utils.enable_purr_mode", lambda enabled=True: None, raising=False)
